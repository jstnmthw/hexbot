...
- **Ollama provider adapter** (`plugins/ai-chat/providers/ollama.ts`): self-hosted, private-by-default alternative to Gemini. Uses Node 20+ native `fetch` against Ollama's `/api/chat` endpoint, so no SDK ships with the bundle. `AIProviderConfig` widened to carry `baseUrl` / `requestTimeoutMs` / `samplingOptions` / `useServerTokenizer` — each provider pulls what it understands. `index.ts` now builds the provider config via a `buildProviderConfig()` dispatch on `cfg.provider` (Gemini pulls `api_key`, Ollama pulls `ollama.base_url`). Token counting defaults to a conservative 4-chars-per-token heuristic; `/api/tokenize` is opt-in via `use_server_tokenizer`. Error mapping treats HTTP 404 (model not pulled) and 400 (bad prompt) as non-retryable `other`, and 5xx / `ECONNREFUSED` / `AbortError` as retryable `network`, so the `ResilientProvider` breaker trips only on genuine infra failures. Startup pings `/api/tags` to fail fast when the daemon is unreachable; a missing-but-pullable model logs a warning instead. Flipping `provider` between `gemini` and `ollama` needs a plugin reload, not a bot restart.
- **Character engine, ambient participation, mood, and social tracking** (`plugins/ai-chat/{character-loader,ambient,mood,social-tracker}.ts`, `plugins/ai-chat/characters/*.json`): the bot behaves as a channel regular rather than a help-desk agent. Characters are JSON-defined (style, chattiness, trigger/avoid keywords, prompt template); ships with 9 presets (`friendly`/`sarcastic`/`chaotic`/`shitposter`/`deadpan`/`minimal`/`gossip`/`nightowl`/`oldhead`). Per-channel character assignment + optional per-channel language via `channel_characters`. Admins can swap at runtime with `!ai character <name>`. Optional channel profiles (`channel_profiles`) append a "this channel is about X, culture is Y" hint to the system prompt. An internal mood engine (energy/engagement/patience/humor) drifts over time and scales response verbosity 0.5×–1.5×. The social tracker classifies activity (`dead`/`slow`/`normal`/`active`/`flooding`) from rolling message counts, tracks unanswered questions per channel, and records back-to-back bot gating. Ambient participation (off by default) uses all of the above to decide whether to speak unprompted: idle remarks in quiet channels, unanswered-question answers, join welcome-backs, topic reactions. A 30s tick loop drives idle/unanswered evaluation; `join`/`topic` binds drive event reactions. All ambient paths respect a separate per-channel / global hourly rate budget.
- **On-demand game sessions** (`plugins/ai-chat/session-manager.ts`, `plugins/ai-chat/games/*.txt`): drop a `.txt` into `games/` and it becomes playable via `!ai play <name>` — the file contents are the session's system prompt. Ships with `20questions` and `trivia`. Sessions are identified by `(nick, channel)` and bound to the creator's services account (or `ident@host` fallback) to defeat nick-takeover hijacks. Session context is a sliding 40-turn window so long games don't scale quadratically. Bypasses the per-user token bucket; global RPM/RPD still apply. Inactivity expiry (10min default) runs on the ambient poll. Hard cap of 500 concurrent sessions with LRU eviction on overflow.
- **Per-user token bucket replaces cooldowns** (`plugins/ai-chat/rate-limiter.ts`): layered rate limiter — integer token bucket per user (burst 3, one refilled every 12s), plus global RPM/RPD sliding windows, plus ambient-specific per-channel/global hourly budgets. When global RPM crosses `rpm_backpressure_pct` (80%), per-user burst halves to avoid storms. `getOrCreateBucket()` lowercases internally as defence-in-depth. Admins (`+m`) bypass the user bucket; global layers always apply.
- **Founder-tier refusal gate** (`plugins/ai-chat/index.ts`, `plugins/chanmod/helpers.ts`): when the bot's ChanServ tier is `founder`, ai-chat refuses to respond by default (`security.disable_when_founder: true`). Checked at trigger time, at pipeline start, and — via a new `gatedSender` wrapper — before every line dispatched to IRC, so a ChanServ probe resolving mid-request or mid-send still blocks the response. Tier is read from the `chanserv_access` chanset written by chanmod's Anope/Atheme auto-detect probe. Operator override: `.chanset <chan> chanserv_access founder|op|none`. See `docs/CHANNEL_PROTECTION.md` for the full rationale and the two-bot topology recommendation.
- **Anope `NO_ACCESS` deferred-commit flow** (`plugins/chanmod/chanserv-notice-anope.ts`): Anope emits `ACCESS info` results and `You are not on the access list` as two separate notices for the same probe; the driver now defers the `no-access` commit until the `INFO` reply arrives so a stale `chanserv_access` chanset can't linger.
- **SAFETY_CLAUSE system-prompt hardening** (`plugins/ai-chat/assistant.ts`): non-overridable safety clause appended to every rendered system prompt. Forbids `.`, `!`, `/` line prefixes (IRC-command execution surface); frames the bot as a regular user with no knowledge of services syntax, mode letters, or ban-mask formats (capability-absence framing — so "what's the command to transfer founder?" gets honest ignorance instead of a working recipe); forbids transcript-format replies (`[nick] msg` tags, multi-speaker dialogue, invented participants).
- **Defense-in-depth fantasy-command dropper** (`plugins/ai-chat/output-formatter.ts`): any LLM line starting with a services-parseable prefix causes the entire response to be dropped with a warn log. Pre-check normalises NFKC, strips `\p{Cf}`/`\p{M}`, and collapses U+2028/U+2029 so invisible-character smuggling and line-separator tricks can't bypass.
- **Resilient provider wrapper** (`plugins/ai-chat/providers/resilient.ts`): bounded exponential backoff with jitter (2 retries on `rate_limit`/`network`), half-open circuit breaker (5 consecutive failures → 5 min open). Failure-counting uses a blacklist (`kind !== 'safety' && kind !== 'auth'`) so new transient error kinds are counted by default; `countTokens()` intentionally bypasses retry. Injectable `now` / `sleep` seams for testability.
- **Gemini provider with request timeout** (`plugins/ai-chat/providers/gemini.ts`): 30s per-request timeout races `generateContent()` so a hung TCP body can't starve the global RPM slot. Maps Gemini SDK errors (`GoogleGenerativeAIFetchError`, `GoogleGenerativeAIResponseError`, 429/401/403/5xx) to `AIProviderError` kinds. Cross-bundle-safe `isAIProviderError()` guard (discriminates by `name` + valid `kind`) replaces `instanceof` checks that broke when the tsup-bundled plugin and a test construct the error from different module copies.
- **Founder-disable default** (`plugins/ai-chat/config.json`, `plugins/chanmod/config.json`): `security.disable_when_founder` defaults `true`. chanmod's default protection posture is op/AOP rather than founder — the two-bot topology (unprivileged AI bot + chanmod-only founder bot) is the recommended deployment when both features are wanted.
- **DI seams for testability** (`src/core/{channel-state,permissions}.ts`, `src/utils/{sliding-window-counter,rate-limit-tracker}.ts`, `src/plugin-loader.ts`, multiple ai-chat modules): constructor `initialState` parameters on `ChannelState`, `Permissions`, `SlidingWindowCounter`, `RateLimitTracker` let tests seed a specific window/bucket without replaying N `record()` calls. `PluginLoader.load(pluginPath, config, deps?)` accepts an optional deps bag so integration tests can inject a mock provider without touching the rest. All seams are backward-compatible and production-inert.

### Changed

- **ai-chat system-prompt assembly restructured into explicit sections** (`plugins/ai-chat/{assistant.ts,characters/types.ts,character-loader.ts,characters/*.json}`): `renderSystemPrompt()` now emits `You are <nick>.` → `## Persona` → `## Right now` → `## Rules (these override Persona and Right now)` instead of concatenating per-character "Rules:" blocks with a SAFETY suffix. Character schema split: `prompt` field renamed to `persona` (just the body, with `{nick}`/`{channel}`/`{network}`/`{users}` placeholders); the in-prompt dash-bullet rules are now a structured `style.notes: string[]` array rendered under Persona. The previously dead `avoids` field is now wired in as a one-line `You avoid topics like: …` statement under Persona. Channel profile no longer needs a placeholder — it's injected automatically into Persona. Drops the `{channel_profile}` template variable (was double-injecting silently when both substitution and the fallback append fired). Removes the dual "Rules:" heading collision that was plausibly contributing to llama3.2:3b imitating the `[nick]` transcript format. All 9 shipped characters migrated. See `docs/prompt-stitching-review.md` for design rationale (Option C of three considered).
...
- **CI runs `pnpm build:plugins` before the test suite** (`.github/workflows/ci.yml`): the `PluginLoader` loads bundled plugins from `plugins/<name>/dist/index.js`, so the integration tests that exercise ai-chat through the real loader see the current source instead of a stale bundle.

### Added

- **Bot-identify state machine** (`src/core/services.ts`): `Services` now tracks a four-state identity (`unknown` → `pending` → `identified` / `unidentified`) rather than a boolean. `identify()` and the SASL fallback path both transition to `'pending'` when IDENTIFY is sent; `'identified'` fires when `account-notify` or a "You are now identified" NickServ notice confirms success. `bot:identified` and `bot:deidentified` are now first-class events on the internal event bus. Plugins can subscribe via `api.onBotIdentified()` / `api.offBotIdentified()`. `api.services.isBotIdentified()` returns the current confirmed state. `BotIdentifyState` is exported for consumers that need the full enum.
- **Nick collision detection + GHOST recovery** (`src/core/connection-lifecycle.ts`, `src/core/services.ts`, `src/core/channel-state.ts`, `src/bot.ts`): after `registered`, the bot reads `client.user.nick` and compares it to the configured nick. A mismatch emits `bot:nick-collision` on the event bus; the synchronous subscriber in `bot.ts` updates `channelState.setBotNick()` and `bridge.setBotNick()` to the real nick before channels are joined. When `irc.ghost_on_recover: true`, `services.ghostAndReclaim()` sends `NickServ GHOST <nick> <password>` then `NICK <nick>` after 1.5 s. `channel-state.onNick` now self-heals `botNick` when the bot's own nick changes, so subsequent join/part/kick tracking is correct after GHOST. New config fields: `irc.alt_nick` (fallback nick for irc-framework's collision handling) and `irc.ghost_on_recover` (enable automatic recovery).
- **IDENTIFY-before-JOIN gate** (`src/core/connection-lifecycle.ts`): when `services.identify_before_join: true`, the bot awaits `bot:identified` before calling `joinConfiguredChannels()`. A max wait of `services.identify_before_join_timeout_ms` (default `10000`) prevents hanging forever when services are down. Eliminates the IDENTIFY/ChanServ probe race on non-SASL networks; composes correctly with GHOST recovery — the bot joins channels only after it is both identified and on its primary nick.

### Fixed

- **2026-04-21 stability audit** (`src/core/services.ts`, `src/core/connection-lifecycle.ts`, `plugins/chanmod/index.ts`, `plugins/chanmod/chanserv-notice.ts`, `src/core/commands/irc-commands-admin.ts`): closed all 10 findings from the post-incident review of the 2026-04-20 network outage, where a silent SASL failure after 9 failed reconnects left the bot unidentified and without channel ops for 12+ hours.
  - **C-1 (SASL silent failure)**: when the bot receives a NickServ "please identify" notice while `sasl: true`, it sends a one-shot password `IDENTIFY` fallback and transitions the identify state to `pending`. Once confirmed, emits `bot:identified`. The "please identify" notice is no longer discarded silently.
  - **C-2 (ChanServ probe fires before identity)**: chanmod subscribes to `bot:identified` and re-probes `verifyAccess()` for every channel where `chanserv_access` is still `'none'`, clearing stale deferred probe state before retrying.
  - **C-3 (STATUS queries fail when bot is unidentified)**: `verifyUser()` now returns `{ verified: false }` immediately when the bot's identify state is `'unidentified'`, avoiding a NickServ round-trip that the server would silently drop.
  - **C-4 (nick collision)**: see Added → nick collision detection + GHOST recovery.
  - **W-1 (IDENTIFY/JOIN race)**: see Added → IDENTIFY-before-JOIN gate.
  - **W-2 (no identify-state observability)**: `botIdentified` is now included in `.status` metrics via `getStabilityMetrics()`; ChanServ probe timeouts promoted from `debug` to `warn` log level.
  - **W-3 (verify timeout too short on reconnect)**: `verifyUser()` uses a 15 s timeout (instead of 5 s) for the first 30 s after `bot:connected`, when NickServ is under heavy load from all reconnecting clients.
  - **W-4 (stale deferred probe state)**: `clearProbeState(channel)` is called before any re-probe to cancel old deferred flushes that would clobber the new result.
  - **I-1 (SASL miss not visible)**: a `WARN` is logged if `registered` fires without a prior `account-notify` on a SASL-configured network (checked after 3 s).
  - **I-2 (no probe on chanmod reload)**: chanmod's `init()` now calls `chain.verifyAccess()` for every configured channel the bot is already present in, so `.reload chanmod` actually restores ChanServ access without requiring a manual `.chanset` override.

- **Ambient tick-loop no longer stealth-disables itself on a throw** (`plugins/ai-chat/ambient.ts`): `tick()` is wrapped in try/catch and `.catch()` handlers for fire-and-forget `sender()` calls route through an optional warn logger (`api.warn`) so an ambient bug surfaces in the log instead of silently leaving the interval running but producing nothing for the process lifetime.
- **Session pipeline silent rate-limit** (`plugins/ai-chat/index.ts`): a rate-limited game session now replies privately with the retry window (matching the public `!ai` path) instead of returning with only a debug log. Provider-null check moved to the top guard alongside the other dependency checks.
- **Engagement map leak** (`plugins/ai-chat/index.ts`): eviction TTL tied to `cfg.triggers.engagement_seconds` (floored at 60s) with an unconditional `>= 1000` cap, so stale entries can't pin below the cap indefinitely.
- **`.ai ignore` unbounded growth** (`plugins/ai-chat/index.ts`): soft cap of 1000 new entries with a clear refusal message. Re-ignoring an existing target is still a no-op.
- **Empty character prompts accepted** (`plugins/ai-chat/character-loader.ts`): `validateCharacter()` now rejects empty/whitespace prompts so a broken JSON file can't ship an unanchored system prompt.
- **Anope `chanserv_access` tier stale after `NO_ACCESS`** (`plugins/chanmod/chanserv-notice-anope.ts`): see Added → deferred-commit flow.
