...
Batches 2, 3, 4, 5 landed in one pass — 39 checkbox items ticked `[x]` below (2 of the 4 CRITICALs, the 4 downgraded CRITICAL/WARNINGs, and the majority of chanmod / plugin-API / defence-in-depth / plugin-polish WARNINGs and a handful of INFOs). Batch 1 (botlink handshake rewrite + BSAY fromHandle + per-frame rate limits + `listen.host` default + RELAY_REQUEST hub gate) is scoped into a dedicated plan at `docs/plans/botlink-handshake-v2.md` — implementation deferred to a follow-up /build on that plan. Post-pass: `pnpm tsc --noEmit` clean, `pnpm vitest run` 3850/3850. Remaining `- [ ]` items are the plan-deferred botlink work plus WARNING/INFO items not in any of the five batches (out-of-scope for this pass, safe to leave for future passes).
