...
**`services_host_pattern` recommendation:** The optional defense-in-depth pattern at `services.services_host_pattern` matches against the **full** sender hostmask (`nick!ident@host`) of any notice claiming to come from NickServ. Pin it to the most specific pattern your network supports — `NickServ!*@services.libera.chat`, not the wildcard `services.*`. On networks that allow user-controlled hostnames, a hostname-only pattern can be matched by a user who picks a hostname starting with `services.`; the full-mask form forces the operator to also think about the ident segment. The same setting feeds chanmod's ChanServ-impostor guard (matched against hostname only there), so the recommended pattern still helps even when only one path applies.

**Plaintext NickServ password risk:** SASL PLAIN over plaintext is a fatal config error (`validateResolvedSecrets`). Two adjacent paths are not fatal but still leak the password in cleartext when `irc.tls=false`: non-SASL `IDENTIFY` (`services.password` set with `services.sasl=false`) and `irc.ghost_on_recover=true` (NickServ GHOST during nick reclaim). Both surface a `[security]` warning at startup; either enable TLS or run the bot through an encrypted tunnel.
...
- Ban duration **doubles on each re-ban** (5m → 10m → 20m → …), capped at 24 hours. The tracker entry decays over time (sweep clears stale entries) **and** on a successful authentication: a successful auth decrements `banCount` so a leaf that connects, fails once, and then reconnects cleanly drifts its escalation back toward zero rather than carrying the failure forever. Persistent scanners that never authenticate stay at the 24h ceiling because they never trip the success-decrement path.
...
- `auth_ip_whitelist` accepts **IPv4** CIDR strings (e.g., `["10.0.0.0/8"]`) and bare IPv4 addresses (expanded to `/32` at runtime) whose IPs bypass all auth rate limiting. IPv6 entries and hostname strings are NOT supported — they surface a `[security]` warning at startup and are dropped from the effective whitelist.
