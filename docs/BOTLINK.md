...
| `auth_ip_whitelist`      | CIDR strings whose IPs bypass auth rate limiting (default `[]`). IPv4 only — IPv6 leaves cannot currently be whitelisted; ban-tracking and rate limits still apply to them.                                                                                                                                                                               |
