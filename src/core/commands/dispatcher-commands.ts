 ...
 */
export function registerDispatcherCommands(handler: CommandHandler, dispatcher: EventDispatcher): void {

  handler.registerCommand('binds', {
    flags: '+o',
    ...
    const lines = binds.map((b) =>
      `  ${b.type} ${b.flags} "${b.mask}" → ${b.pluginId} (hits: ${b.hits})`
    );
